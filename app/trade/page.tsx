...
const highlightText = (text: string, query: string) => {
  ...
  return parts.map((part, i) =>
    part.toLowerCase() === query.toLowerCase() ? (
      <span key={i} className="text-[#22ff88]">
        {part}
      </span>
    ) : (
      part
        ...
        <div className="flex flex-col items-center">
          ...
          <span
            className={`text-[15px] font-semibold tracking-wide ${
              isBullish ? "text-[#22ff88]" : "text-[#ff5577]"
            }`}
          >
...
useEffect(() => {
  const onVisibilityChange = () => {
    if (document.visibilityState === "hidden") {
      clearAllIntervals();
      console.log("[PERF] Tab hidden — all intervals stopped");
    }
  };
  ...
  const id = safeInterval(() => {
    setLiveData((prev) =>
      prev.map((arr) => {
        ...
        const clamp = Math.max(10, Math.min(150, next));
        return [...arr.slice(1), clamp];
      })
    );
  }, 1100);
...
{showExitGuard && (
  ...
  ">
    ...
    <div className="text-gray-400 text-[12px]">
      System is already calibrated for your risk profile.
    </div>
  </div>
)}
